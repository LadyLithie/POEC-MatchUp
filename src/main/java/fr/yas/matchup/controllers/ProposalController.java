	...
	public void initView() {
		...
		if (job == null) { //mode creation
			...
			if (user instanceof Enterprise) {
				...
				vFrame.getComboBox_linkedUser().setSelectedItem(job.getCompagny());
				
			}else {
				vFrame.getComboBox_linkedUser().addItem(job.getHeadhunter());
				vFrame.getComboBox_linkedUser().setSelectedItem(job.getHeadhunter());
				
				JLabel lblCompagny = new JLabel("Entreprise");
				GridBagConstraints gbc_lblCompagny = new GridBagConstraints();
				gbc_lblCompagny.insets = new Insets(0, 0, 0, 5);
				gbc_lblCompagny.gridx = 0;
				gbc_lblCompagny.gridy = 3;
				vFrame.getPanelCheckBox().add(lblCompagny, gbc_lblCompagny);
				JComboBox<RegisteredUser> comboBox_company = new JComboBox<RegisteredUser>();
				comboBox_company.addItem(job.getCompagny());
				comboBox_company.setSelectedItem(job.getCompagny());
