...
public class MatchingCode {
	...
	public double matching() {
		...
		for (int i = 0 ; i < job.getProposalSkills().size() ; i++) {
			jobSkills.add(i, job.getProposalSkills().get(i));
		}
		...
		for (int i = 0; i < jobSkills.size(); i++) {
			for (int j = 0; j < candidate.getCandidateSkills().size(); j++) {
				...
				if ( candidate.getCandidateSkills().get(j).getName() == job.getProposalSkills().get(i).getName() ) {
					
					matchedSkills.add(i, job.getProposalSkills().get(i));
